...
$psw_orders = $panel["panel_limit_order"]-$panel["panel_orders"];
  ...
  if( route(0) != "admin" &&  route(0) != "psw_rental" && $settings["site_maintenance"] == 1 ): include 'app/views/maintenance.php';exit(); endif;
 ...
 $unpaidCount =  countRow(["table"=>"invoices","where"=>["invoice_status"=>"Unpaid" , "client_id" => $user["client_id"]]]);
	...
	if( $_SESSION["msmbilisim_userlogin"] != 1  || $user["client_type"] == 1  ){
      ...
      array(
            ...
            'ticketCount'=>new_ticket($user['client_id']),'paymentsList'=>$methodList,'integrationsList'=>$methoList,'transactions'=>$transaction_logs,'chilpanel_logs'=>$panel_logs,'decoration'=>$decoration,'Pswphonepe'=>$phonepe["method_type"],'PaytmQR'=>$PaytmQR["method_type"],'PaytmQRimage'=>$PaytmQRimage,'WhatsApp'=>$WhatsApp["method_type"],'whatsappstatus'=>$whatsappstatus,'whatsappvisibility'=>$whatsappvisibility,'bankPayment'=>$bankPayment["method_type"],'bankList'=>$bankList,
 ...
 $unpaidCount =  countRow(["table"=>"invoices","where"=>["invoice_status"=>"Unpaid" , "client_id" => $user["client_id"]]]);
	 echo $twig->render( $templateDir.'.twig',
      array(
            ...
            'styleList'=>$stylesheet["stylesheets"],'scriptList'=>$stylesheet["scripts"],'captchaKey'=>$settings["recaptcha_key"],'captcha'=>$captcha,'resetPage'=>$resetPage,'serviceCategory'=>$categories,'decoration'=>$decoration,'pswdecoration'=>$pswdecoration,'categories'=>$categories,'category_icon'=>$categories['category_icon'],
            'error'=>$error,'errorText'=>$errorText,'success'=>$success,"servicesPage"=>$serviceList,"resetType"=>$resetType,'successText'=>$successText,'title'=>$title,"Signup"=>$Signup,"ChildPanel"=>$ChildPanel,'affiliatessystem'=>$affiliatessystem,'Promotion'=>$Promotion,
